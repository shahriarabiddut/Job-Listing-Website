...
@section('content')
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">Editing Recruiter: {{ $data->name }}</h1>

    <!-- DataTales Example -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Edit Recruiter
            <a href="{{ url('admin/recruiter/'.$data->id) }}" class="float-right btn btn-success btn-sm"> <i class="fa fa-arrow-left"></i> View All </a> </h6>
        </div>
        <div class="card-body">
            
            <div class="table-responsive">
            <form method="POST" action="{{ route('admin.recruiter.update',$data->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
